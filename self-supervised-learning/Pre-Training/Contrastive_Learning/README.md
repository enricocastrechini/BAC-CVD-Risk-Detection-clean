...
The original project used preprocessed LIDC CT slices. This repository's
adaptation supplies a mammography data directory instead, but the exact
loader layout must be checked against the selected module before a full
run. The committed BYOL loader uses `ImageFolder`, so an unlabeled dataset
still needs class-like subdirectories, for example:
...
Do not assume that every upstream augmentation, crop size or channel
convention is valid for mammograms.
