...
`BAC_DATA_DIR` must point to the local image directory expected by the
selected loader. The loaders do not share one universal contract:
SparK expects an ImageNet-style tree with `train/` and `val/`
subdirectories, while the committed BYOL dataset subclasses
`torchvision.datasets.ImageFolder` and therefore also needs class-like
subdirectories. `BAC_OUTPUT_DIR` is optional and defaults to a local
`results/` folder. Use `WANDB_MODE=offline` for local logging; provide
`WANDB_API_KEY` only via the environment when online logging is required.
...
The script uses ConvNeXt-Small, mammography input dimensions `1120 x
576`, 800 epochs and the optional `BAC_RESUME_CHECKPOINT`. The current
SparK entry point imports `utils.arg_util`, which is not present in the
checked-in MAE `utils/` directory, so this path requires completing that
dependency before it can run. For contrastive pretraining, only the BYOL
launcher is committed; SwAV and MoCoV2 require manual module invocation
and validation of their inherited upstream assumptions.

## Downstream evaluation

Open the relevant notebook in `Downstream/` after installing its
`requirements.txt`. The committed notebooks are inherited Brain,
COVID-19 and OrgMNIST examples, not BAC-specific evaluation notebooks.
Adaptation to the authorized labeled BAC split must be recorded rather
than inferred from the notebook filenames. Keep the encoder checkpoint,
notebook parameters, seed and dependency versions with every reported
result.
